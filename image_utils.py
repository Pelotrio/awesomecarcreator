...
def change_color(image, color):
    ...
    color_h = Image.new("RGB", (1, 1), color).convert("HSV").getpixel((0,0))[0]

    image_h = numpy.full((image.width, image.height), color_h)
...
def create(template_name: str, color: QColor):
    ...
    template = change_color(template, color.getRgb()[:3]) # (255, 0, 127)

    template = jpegify(template)
    template = ruin_resolution(template)

    qt_image = ImageQt(template)
    pixmap = QPixmap.fromImage(qt_image)

    print('DEBUG: (SLOW FUNCTION) done creating cursed image')
